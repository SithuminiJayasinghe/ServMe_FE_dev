...
@Component({
  templateUrl: 'itemspanelreport.component.html',
  providers: [
...
@NgModule({
  ...
  declarations: [
    ItemsPanelReportComponent,

  ]
})
export class ItemsPanelReportComponent implements OnInit, OnDestroy {
  ...
  async doActiveRate(val) {

    this.http.get<any>("http://localhost:5102/item/getByRateItemById?id="+val
  )
    .subscribe(
        (val) => {
            console.log("POST call successful value returned in body", 
                        val);
                        alert("Item rate : " + val.rateCount);
    ...
    // console.log("GET");
    // let url = `http://localhost:5102/item/getByRateItemById?id=`+val;
    // this.http.get(url).subscribe(res => 
      // console.log(res.rateCount),
...
// alert();
  ...
  doActive(val) {
    ...
    this.http.post("http://localhost:5102/item/active",
    {
      "id":val,
    }
  )
  ...
  doInactive(val) {
    ...
    console.log("POST");
    console.log("this.search.name - " + this.search.city_id);
    this.http.post("http://localhost:5102/item/inactive",
    {
      "id":val,
    }
  )
